...
class RunningMeanStd(object):
  ...
  def update_from_moments(self, batch_mean, batch_var, batch_count):
    ...
    self.mean = new_mean.astype(self.dtype)
    self.var = new_var.astype(self.dtype)
...
class Normalizer:
  ...
  def __call__(self, training, *args, **kwargs):
    if training:
...
class MeanStdNormalizer(BaseNormalizer):
  def __init__(self, read_only=False, clip_before=200.0, clip_after=10.0, epsilon=1e-8):
