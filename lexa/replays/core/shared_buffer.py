...
from replays.core.replay_buffer import RingBuffer
...
def past_samples(idxs):
  """Assumes there is an 'ag' field, and samples n transitions, pairing each with a future ag
  from its trajectory."""
  global BUFF

  trights = BUFF.buffer_tright.get_batch(idxs)
  future_idxs = idxs
  idxs = idxs - np.round(np.random.uniform(size=len(idxs)) * trights).astype(np.int64)
  ...
  # add random future goals
  ags = BUFF.buffer_ag.get_batch(future_idxs)
  transition.append(ags)
  return transition

def close_samples(idxs, distance):
  """Assumes there is an 'ag' field, and samples n transitions, pairing each with a future ag
  from its trajectory."""
  global BUFF

  # the number of elements in the left
  trights = BUFF.buffer_tright.get_batch(idxs)
  trights = np.minimum(trights, distance)
  future_idxs = idxs
  idxs = idxs - np.round(np.random.uniform(size=len(idxs)) * trights).astype(np.int64)
...
class SharedMemoryTrajectoryBuffer():
  ...
  def __init__(self, limit, item_shape, n_cpu=1):
    ...
    BUFF.raw_tright = RawArray('d', limit)
    BUFF.np_tright = np.frombuffer(BUFF.raw_tright, dtype=np.int64)
    BUFF.buffer_tright = RingBuffer(limit, shape=(), dtype=np.int64, data=BUFF.np_tright)
  ...
  def add_trajectory(self, *items):
    ...
    self.BUFF.buffer_tright.append_batch(np.arange(trajectory_len, dtype=np.int64))
  ...
  def sample_past(self, batch_size, batch_idxs=None):
    if batch_idxs is None:
      batch_idxs = np.random.randint(self.size, size=batch_size)

    if self.pool is not None:
      res = self.pool.map(past_samples, np.array_split(batch_idxs, self.n_cpu))
      res = [np.concatenate(x, 0) for x in zip(*res)]
      return res

    return past_samples(batch_idxs)
  
  def sample_close(self, batch_size, distance=10, batch_idxs=None):
    if batch_idxs is None:
      batch_idxs = np.random.randint(self.size, size=batch_size)

    return close_samples(batch_idxs, distance)
  
  def sample_close_to_sparse(self, batch_size, ag_kde, distance=10, batch_idxs=None):
    if batch_idxs is None:
      batch_idxs = np.random.randint(self.size, size=batch_size*5)
    
    if self.pool is not None:
      ags = self.pool.map(ag_samples, np.array_split(batch_idxs, self.n_cpu))
      ags = [np.concatenate(x, 0) for x in zip(*ags)]
    else:
      ags = ag_samples(batch_idxs)
    
    # Get the density module.
    # Quite simple and straightforward
    if len(ags) > 0:
      sampled_ag_scores = ag_kde.evaluate_log_density(ags)
      batch_idxs = batch_idxs[np.argsort(sampled_ag_scores) < batch_size]
    # Then we perform the close sampling.
    
    # if self.pool is not None:
    #   res = self.pool.map(close_samples, np.array_split(batch_idxs, self.n_cpu))
    #   res = [np.concatenate(x, 0) for x in zip(*res)]
    #   return res
